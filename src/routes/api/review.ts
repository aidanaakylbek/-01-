  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (isRateLimited(`review:${dashboard.account.id}`, 15, 60_000)) {
          return Response.json(
        ...
        try {
          ...
          if (!review) {
            if (lastError) {
              console.error("AI-Sana AI review fallback failed", lastError);
            }
            return Response.json(
          ...
          console.error("AI-Sana AI review error", error);
          return Response.json(
